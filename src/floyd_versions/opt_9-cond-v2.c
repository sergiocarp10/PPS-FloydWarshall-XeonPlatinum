...
void floydWarshall(TYPE* D, int* P, int n, int t){
	...
	F = (F_MATRIX_TYPE**) abs_malloc(2 * sizeof(F_MATRIX_TYPE*));
	for (x=0; x<2; x++) F[x] = (F_MATRIX_TYPE*) abs_malloc(r * sizeof(F_MATRIX_TYPE));

	cv = (pthread_cond_t**) malloc(2 * sizeof(pthread_cond_t*));
	for (x=0; x<2; x++) cv[x] = (pthread_cond_t*) malloc(r * sizeof(pthread_cond_t));

	mutex = (pthread_mutex_t**) malloc(2 * sizeof(pthread_mutex_t*));
	for (x=0; x<2; x++) mutex[x] = (pthread_mutex_t*) malloc(r * sizeof(pthread_mutex_t));
		...
		for(k=0; k<r; k++){
			...
			for(w=0; w<r*2; w++){
				if(w<r){ 
					...
					//for (aux=0; aux<r; aux++){
						...
						pthread_mutex_lock(&mutex[0][j]);
						F[0][j]--;
						pthread_cond_broadcast(&cv[0][j]);
						pthread_mutex_unlock(&mutex[0][j]);
					//}
					...
					//for (aux=0; aux<r; aux++) {
						...
						pthread_mutex_lock(&mutex[1][i]);
						F[1][i]--;
						pthread_cond_broadcast(&cv[1][i]);
						pthread_mutex_unlock(&mutex[1][i]);
					//}
			...
			for(i=0; i<r; i++){
				for(j=0; j<r; j++){
					...
					pthread_mutex_lock(&mutex[0][j]);
					if (F[0][j] > 0) pthread_cond_wait(&cv[0][j], &mutex[0][j]);
					pthread_mutex_unlock(&mutex[0][j]);
					
					pthread_mutex_lock(&mutex[1][i]);
					if (F[1][i] > 0) pthread_cond_wait(&cv[1][i], &mutex[1][i]);
					pthread_mutex_unlock(&mutex[1][i]);
